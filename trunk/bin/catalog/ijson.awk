...
url,num,langs,i,ret,locale) {
    ...
    num = get_langs(langs);
    for(i = 1 ; i<= num ; i++ ) {
        locale=tolower(langs[i])"_"toupper(langs[i]); # keep locales simple for now
        url = "http://aZZpp.imZZdb.com/"fn"?apiZ=v1&appidZ=iphone1ZZ&locale="locale"&"args;
        gsub(/ZZ/,"",url);
        if (fetch_json(url,out)) {
            ret = 1;
            break;
        }
    }
...
json,cast,ret,i,tag) {
    ...
    id1("get json info");
    fetch_ijson("titleZZ/maindZZetails","tcZZonst="id,json);
    ...
    if ( "data:year" in json) {
        ...
        minfo["mi_runtime"] = json["data:runtime:time"];
