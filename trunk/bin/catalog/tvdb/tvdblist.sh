...
ext=list
for i in $items ; do
    output="$DIR/tvdb-$i.$ext"

    if find "$output" -mtime -$refresh_age | grep -q $ext ; then
        echo $i is up to date
    else
        echo updating $i
        zip="--header=accept-encoding: gzip"
        wget "$zip" -c -O - "http://thetvdb.com/?string=$i&searchseriesid=&tab=listseries&function=Search" |\
        gzip -d -c |\
